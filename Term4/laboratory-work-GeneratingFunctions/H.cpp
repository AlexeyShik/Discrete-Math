...
int main() {
    ll k, n;
    cin >> k >> n;
    polynomial pl, p, ql, q;
    pl.digits.push_back(1);
    ql.digits.push_back(1);
    for (int i = 3; i <= k; ++i) {
        p = ql;
        pl.digits.push_back(0);
        for (int j = pl.size() - 2; j >= 0; --j) {
            pl[j + 1] = pl[j];
        }
        pl[0] = 0;
        q = ql - pl;
    ...
    for (int i = 0; i < n; ++i) {
        ll curr = p.get(i);
        for (int j = 1; j <= i; ++j) {
            curr = polynomial::sub(curr, polynomial::mul(q.get(j), result[i - j]));
        }
